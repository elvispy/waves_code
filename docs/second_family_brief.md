...
**Follow-up note (2026-04-18, Julia beam-end uncoupled branch):** On the Julia-native uncoupled beam-end branch, both the top-20 highest-$EI$ points and the bottom-15 lowest-$EI$ points satisfy $|S|/|A| \ll 1$, while $\cos(\arg S - \arg A)$ is never small: $|\cos(\arg S - \arg A)| \approx 1$ throughout both subsets. So for this beam-end formulation, the branch is not explained by phase orthogonality ($\arg S - \arg A \approx \pm \pi/2$); it is an $S \approx 0$ branch in both the high-$EI$ and low-$EI$ regimes.

**Follow-up note (2026-04-18, intersection caveat):** Across the full uncoupled beam-end branch, the only clear breakdown of the $|S|/|A| \ll 1$ pattern is the family-intersection pocket near $\log_{10}(EI) \approx -3.8$. Excluding that pocket, the branch remains uniformly $S$-small (median $|S|/|A| \approx 8\times10^{-3}$, no points above $0.1$).
...
**Follow-up note (2026-04-18, Julia beam-end uncoupled branch):** The uncoupled beam-end branch shows a cleaner split than the historical domain-end branch. On the top-20 highest-$EI$ points, the symmetric part is governed primarily by endpoint cancellation between modes 0 and 2, with higher symmetric modes acting as small corrections. On the bottom-15 lowest-$EI$ points, the branch still satisfies $S \approx 0$, but the total energy is stored mainly in antisymmetric elastic mode 3 (mean energy fraction $\approx 0.75$), while the symmetric cancellation is no longer as cleanly reducible to a pure $0+2$ picture. So the qualitative mechanism survives at low $EI$, but the low-order closure weakens.
...
### Coupled Case: A Posteriori Verification

As of April 19, 2026, the coupled branch extraction infrastructure is "Cluster Ready". The goal is to extract high-fidelity points along the $d=0.03$ $\alpha=0$ curve and verify the *a posteriori* modal cancellation:
$$
S_{\text{mech}} + S_{\text{hydro}} \approx 0
$$
where $S_{\text{hydro}}$ explicitly accounts for the pressure projection $Q_n$. Preliminary smoke tests on a 5x5 grid confirm a ~90% cancellation efficiency, validating the framework before full-scale cluster execution.

## Conclusion: The A Priori Law

As of April 19, 2026, the **uncoupled (d=0) second family** is fully understood. The non-vertical zero-crossing curves for the symmetric ($S \approx 0$) branch are governed by the following analytical *a priori* law:

$$\mathbf{W}_{\text{end}}^T \mathbf{D}^{-1} \mathbf{G}^{-1} \mathbf{W}(x_M) \approx 0$$

where $W_n$ are the free-free beam modes and $\mathbf{G}$ is the discrete Gram matrix. Numerical verification confirms this law predicts the first nontrivial branch with **< 2% error** across 4 decades of $EI$.

The "smog" surrounding the previous failures was identified as:
1.  **Ghost Coupling:** Inadvertent use of $d > 0$ in the solver due to default parameter overrides.
2.  **Basis Normalization:** Misalignment between the discrete orthonormal basis ($\Psi_n$) used in the solver and the continuous analytical basis ($W_n$) assumed in simple theories.

With these facts established, the roadmap is clear:
-   [x] Abandon $\Psi_n$ for theoretical work; use $W_n$ with explicit normalization.
-   [x] Use the verified uncoupled law as the backbone for the full hydrodynamic ($d > 0$) theory.
-   [ ] Run full high-fidelity extraction of the coupled branch on the cluster (100 points).
-   [ ] Verify the coupled *a posteriori* modal balance ($S_{\text{total}} \approx 0$).
-   [ ] Incorporate the off-diagonal hydrodynamic coupling matrix $\mathbf{H}$ into the same implicit framework to crack the coupled case *a priori*.

## Data and code

| Item | Path |
|------|------|
| Uncoupled Details (W-basis) | `Julia/output/single_alpha_zero_curve_details_uncoupled_refined_w_basis.csv` |
| Coupled Details (W-basis) | `Julia/output/single_alpha_zero_curve_details_refined_w_basis.csv` |
| Point Diagnostics Cache | `Julia/output/second_family_point_cache.jld2` |
...
| Resonance theory (first family) | `Resonance distilled 2.md` |

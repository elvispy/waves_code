...
# (H exponent set to zero, but including non-linear displacement scaling)
...
function main()
    ...
    for i in 1:n_pts
        ...
        q0_num = abs(modal.q_w[1])
        
        push!(data_results, (L=L, d=d, omega=2pi*f, q0=q0_num, Qf=Qf_num))
    end
    
    # 1. Calibrate C (Intercept)
    log_C_vals = [log(res.Qf) - (-1.27*log(res.L) - 0.11*log(res.d) - 0.28*log(res.omega) - 0.14*log(res.q0)) for res in data_results]
    C_opt = exp(mean(log_C_vals))
    
    # 2. Calculate errors
    errors = Float64[]
    for res in data_results
        Qf_pred = C_opt * (res.L^-1.27) * (res.d^-0.11) * (res.omega^-0.28) * (res.q0^-0.14)

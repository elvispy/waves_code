function [U, x, z, phi, eta, args] = flexible_surferbot_v2_modified(varargin)
    % Parse inputs
    p = inputParser;
    
    % --- Physical constants ---
    addParameter(p, 'sigma', 72.2e-3);              % [N/m] surface tension of water
    addParameter(p, 'rho', 1000.);                  % [kg/m^3] density of water
    addParameter(p, 'omega', 2*pi*10.);             % [rad/s] driving frequency (80 Hz)
    addParameter(p, 'nu', 1e-6);                    % [m^2/s] kinematic viscosity of water
    addParameter(p, 'g', 9.81);                     % [m/s^2] gravitational acceleration

    % --- Raft properties ---
    addParameter(p, 'L_raft', 0.05);                   % [m] length of the raft
    addParameter(p, 'motor_position', 0.6/2.5 * 0.05); % [m] motor position along the raft (fraction x L_raft)
    addParameter(p, 'd', nan);                        % [m] depth of surferbot (third dimension, z-direction)
    addParameter(p, 'EI', nan); % [N m^2] bending stiffness
    addParameter(p, 'rho_raft', nan);           % [kg/m] mass per unit length of the raft

    % --- Domain settings ---
    addParameter(p, 'L_domain', nan);               % [m] total length of the simulation domain
    addParameter(p, 'domainDepth', nan);            % [m] depth of the simulation domain (second dimention, y-direction)

    % --- Solver settings ---
    addParameter(p, 'n', nan);                      % [unitless] number of grid points in x in the raft
    addParameter(p, 'M', nan);                      % [unitless] number of grid points in z
    addParameter(p, 'ooa', 4);                      % [unitless] finite difference order accuracy
    addParameter(p, 'test', false);                 % [boolean]  whether to run self-diagnostic tests
    % --- Motor parameters ---
    addParameter(p, 'loads', nan);
    addParameter(p, 'motor_inertia', 0.13e-3 * 2.5e-3);  % [kg/m^2] motor rotational inertia
    addParameter(p, 'motor_force'  , nan);              % [N] Strength of the motor force if not provided by intertia
    addParameter(p, 'forcing_width', 0.05);             % [fraction of L_raft] width of Gaussian forcing

    % --- Boundary condition ---
    addParameter(p, 'BC', 'radiative');             % Boundary condition type (radiative, Neuman, Dirichlet)
    

    parse(p, varargin{:});
    args = p.Results;
    %args.ooa = 4; % Define finite difference accuracy in space


    % testing values
    if isnan(args.n); args.n = 20; end
    if isnan(args.L_domain); args.L_domain = 20; end
    if isnan(args.domainDepth); args.domainDepth = 20; end
    if isnan(args.M); args.M = 20; end

    % Grid Construction
    N = round((args.n-1) * args.L_domain / args.L_raft)+1;
    L_domain_adim = N / args.n; 
    M = args.M;
    x = linspace(-L_domain_adim/2, L_domain_adim/2, N);
    dx = x(2) - x(1);

    L_c   = args.L_raft;
    z = linspace(-args.domainDepth, 0, M) / L_c;
    dz = abs(z(2) - z(1));

    % Contact and free indices
    x_contact = abs(x) <= args.L_raft / (2 * L_c);
    x_free    = abs(x) >  args.L_raft / (2 * L_c);
    x_free(1) = false; x_free(end) = false;

    % Derived parameters
    if isnan(args.motor_force)
        force = args.motor_inertia * args.omega^2;
    else
        force = args.motor_force;
    end
    if isnan(args.d); args.d = 0.6 * args.L_raft; end
    if abs(args.motor_position) > args.L_raft/2
        args.motor_position = sign(args.motor_position) * args.L_raft/2; 
        warning('motor position rescaled to inside the raft');
    end

    % New Default Vectors
    if isnan(args.rho_raft);args.rho_raft = repmat(0.052, sum(x_contact), 1); end
    if isnan(args.EI); args.EI = repmat(3.0e9 * 3e-2 * 9e-4^3 / 12, sum(x_contact), 1); end
    
    t_c   = 1 / args.omega;
    m_c   = sum(args.rho_raft) * dx; 
    F_c   = m_c * L_c / t_c^2;

    % --- Non-dimensional groups ---
    Gamma  = args.rho * args.L_raft^2 / (m_c / args.L_raft); 
    Fr     = sqrt(args.L_raft * args.omega^2 / args.g);
    Re     = args.L_raft^2 * args.omega / args.nu;
    kappa  = min(args.EI) / (m_c / args.L_raft * args.L_raft^4 * args.omega^2); 
    We     = (m_c / args.L_raft) * args.L_raft * args.omega^2 / args.sigma; 
    Lambda = args.d / args.L_raft; 
    
    % vectors
    EI_adim = args.EI / (F_c * L_c^2);
    rho_raft_adim = args.rho_raft * L_c / m_c;

    nd_groups = struct( ...
        'Gamma',  Gamma, ...
        'Fr',     Fr, ...
        'Re',     Re, ...
        'kappa',  kappa, ...
        'We',     We, ...
        'Lambda', Lambda);

    % Store nondimensional groups for later use
    args.nd_groups = nd_groups;

    args.k = NaN; 
    while isnan(args.k) || tanh(real(args.k) * args.domainDepth) < 0.99
        
        % Wavenumber
        k = dispersion_k(args.omega, args.g, args.domainDepth, args.nu, args.sigma, args.rho);
        args.k = k;
        
        if isnan(args.domainDepth) 
            args.domainDepth = 2.5*args.g/args.omega^2; 
        else
            args.domainDepth = 1.05 * args.domainDepth;
        end
    end
    
    % if tanh(args.k * args.domainDepth) < 0.95; warning('Domain depth not enough for dispersison relation'); end
    
    res = 80; % Minimal resolution per unit 
    if isnan(args.n) 
        args.n = max(res, ceil(res / (2*pi/real(k)) * args.L_raft));
        args.n = args.n + mod(args.n, 2) + 1;
    end
    if 2*pi/real(k) / (args.L_raft / args.n) < res; warning('Number of points in x direction too small. Consider changing n to %d', ceil(res / (2*pi/real(k)) * args.L_raft)); end
    if isnan(args.M) 
        args.M = ceil(res * real(k) * args.domainDepth);
    end
    if 1/real(k) / (args.domainDepth / args.M) < res; warning('Number of points in z direction too small. Consider changing M to %d', ceil(res / (2*pi/real(k)) * args.L_raft)); end
    
    if isnan(args.L_domain) %|| args.L_domain < 10 * 2*pi/k 
        args.L_domain = min(3 * args.L_raft, round(20*2*pi/real(k) + args.L_raft, 2, 'significant')); 
    end
    

    % Loads at which the motor applies a force to the raft
    if isnan(args.loads)
        loads_adim = force / F_c * gaussian_load(args.motor_position/L_c, args.forcing_width, x(x_contact));
    else
        loads_adim = args.loads * L_c / F_c;
    end

    if args.test
        assert(abs(sum(dx * loads_adim) - force/F_c) < 1e-12);
    end
    
    % Solve system
    solution = build_system_v2_modified(N, M, dx, dz, x_free, x_contact, loads_adim, EI_adim, rho_raft_adim, args);
    
    %% Post-processing
    phi = reshape(solution(1:(N*M)), M, N);
    phi_z = reshape(solution((N*M+1):end), M, N);

    x = x * L_c;
    z = z * L_c;    
    args.x_contact = x_contact; args.x = x;
    args.loads = loads_adim * F_c/L_c;
    %figure(5); semilogy(x(x_contact), args.loads); hold on;
    args.N = N; args.M = M; args.dx = dx * L_c; args.dz = dz * L_c;
    args.t_c = t_c; args.L_c = L_c; args.m_c = m_c;

    [U, power, thrust, eta, p] = calculate_surferbot_outputs(args, phi, phi_z);
    
    args.pressure = p;
    args.k = k; args.power = power; args.thrust = thrust;
    phi = full(reshape(phi * L_c^2 / t_c, M, N));
    args.phi_z = full(reshape(phi_z * L_c / t_c, M, N));
    
end
 function [xsol, A, b] = build_system_v2_modified(N, P, dx, dz, x_free, ...
                            x_contact, motor_weights, EI_adim, rho_raft_adim, args)
...
idxLeftFreeSurf = [P; idxFreeSurf(1:(end/2)); find(contactMask, 1, 'first')]; 
nbLeft          = nnz(idxLeftFreeSurf);
idxRightFreeSurf= [find(contactMask, 1, 'last'); idxFreeSurf(((end/2)+1):end); NP];
...
% NOTE: Changed This
S2D{1, 1}(CC, CC) = 1.0i * Lambda * Gamma * dx^2 * I_NP(CC, CC) + 2*Gamma*Lambda / Re * Dx2Raft; %phi
S2D{1, 2}(CC, CC) = (1.0i - 1.0i * Gamma * Lambda/Fr^2) * dx^2 * I_NP(CC, CC) * rho_raft_adim; 
S2D{1, 3}(CC, :)  =  Dx2Raft;
...
% identity = vector * dxx (EI(x) now a vector)
S2D{3, 2}(:, CC) = + 1.0i * Dx2Raft;
S2D{3, 3} = (dx^2/EI) * I_CC * EI_adim;  

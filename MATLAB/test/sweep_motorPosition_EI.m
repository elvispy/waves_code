function S = sweep_motorPosition_EI(saveFile, draftValue)
% Sweep motor_position and EI. Retry runs with higher resolution if tail_flat_ratio > 0.05.

if nargin < 1 || isempty(saveFile)
    saveFile = 'data/sweepMotorPositionEI2.mat';
end
if nargin < 2 || isempty(draftValue)
    draftValue = 0;
end
...
base = struct( ...
    ...
    'L_raft',L_raft, 'motor_position',0.24*L_raft/2, 'd',draftValue, ...
...
proto = struct('thrust_N',NaN,'N_x',NaN,'M_z',NaN, ...
               ...
               'eta_left_domain',NaN,'eta_right_domain',NaN, ...
               'eta_left_beam',NaN,'eta_right_beam',NaN, ...
               'eta_beam_ratio',NaN, ...
...
for ip = 1:numel(motor_position_list)
    ...
    parfor ie = 1:numel(EI_list)
        ...
        S(ip,ie).eta_left_domain = R.eta_left_domain;
        S(ip,ie).eta_right_domain = R.eta_right_domain;
        S(ip,ie).eta_left_beam   = R.eta_left_beam;
        S(ip,ie).eta_right_beam  = R.eta_right_beam;
        S(ip,ie).EI              = p.EI;
        S(ip,ie).motor_position  = p.motor_position;
        S(ip,ie).Sxx             = R.Sxx;
        S(ip,ie).eta_edge_ratio  = R.eta_edge_ratio;
        S(ip,ie).eta_beam_ratio  = R.eta_beam_ratio;
...
function S = run_once(p)
    ...
    S = struct( ...
        ...
        'eta_1', metrics.eta_left_domain, ...
        'eta_end', metrics.eta_right_domain, ...
        ...
        'args', args, ...
        'Sxx', Sxx, ...
        'eta_edge_ratio', metrics.eta_domain_ratio, ...
        'eta_beam_ratio', metrics.eta_beam_ratio);
end

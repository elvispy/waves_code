...
base = struct( ...
    ...
    'EI',10*3.0e9*3e-2*(9.9e-4)^3/12, 'rho_raft',0.018*3.0, ...
    'domainDepth',0.2, 'n',101, 'M',600, ...
...
for i = 1:numel(S)
    semilogy(S(i).x, abs(S(i).eta), 'DisplayName', sprintf('dx/k=%.2g', S(i).args.dxS(i).args.k));
end

function plot_sweep_motorPosition_EI_beam_end_uncoupled(saveDir)
%PLOT_SWEEP_MOTORPOSITION_EI_BEAM_END_UNCOUPLED Plot the x_M-EI asymmetry
%figure using beam-end amplitudes from the uncoupled d = 0 sweep dataset.
...
for i = 1:numel(required)
    if ~isfield(S, required{i})
        error('plot_sweep_motorPosition_EI_beam_end_uncoupled:MissingField', ...
...
title(ax, 'd = 0 (uncoupled), beam-end asymmetry', 'FontSize', 14);
set(ax, 'FontSize', 13, 'LineWidth', 0.75, 'TickDir', 'out', 'Box', 'on');
grid(ax, 'on');

outname = fullfile(saveDir, 'motorPositionEI_beam_end_uncoupled.pdf');
outname_png = fullfile(saveDir, 'motorPositionEI_beam_end_uncoupled.png');

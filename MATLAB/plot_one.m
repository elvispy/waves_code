...
[out{1:numel(names)}] = flexible_surferbot_v2( ...
    ...
    'motor_position', 0.3*0.05/2   , ...   % [m] motor x-position (from -L/2 to L/2)
    'd'             , 0.05/2         , ...   % [m] raft depth (spanwise)
    'EI'            , 3.0e9 * 3e-2 * (9.9e-4)^3 / 12 , ...  % [N m²] bending stiffness
    'rho_raft'      , 0.018*10.0    , ...   % [kg m?¹] linear mass
    'L_domain'      , 0.5          , ...   % [m] domain length
    'domainDepth'   , 0.5          , ...   % [m] water depth
    'n'             , 401          , ...   % grid points in the raft
    'M'             , 400          , ...   % gird points in the z direction

...
[out{1:numel(names)}] = flexible_surferbot_v2( ...
    ...
    'L_domain'      , 5*L_raft      , ...
    'motor_position', 0.0*L_raft/2  , ...   % [m] motor x-position (from -L/2 to L/2)
    'd'             , 0.03         , ...   % [m] raft depth (spanwise)
    'EI'            , Inf*3.0e9 * 3e-2 * (9.9e-4)^3 / 12 , ...  % [N m²] bending stiffness
    'rho_raft'      , 0.052        , ...   % [kg/m] linear mass 
    'loads'         , linspace(-1, 1, 301), ... %'domainDepth'   , 10*L_raft     , ...   % [m] water depth %
    'n'             , 301          , ...   % grid points in the raft %'M'             , 200          , ...   % gird points in the z direction

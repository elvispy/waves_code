...
[out{1:numel(names)}] = flexible_surferbot_v2( ...
    ...
    'omega'         , 2*pi*85.9919563319064     , ...   % [rad 1/s] drive frequency
    'nu'            , 1.0e-6      , ...   % [m2/s] kinematic viscosity
    'g'             , 9.81          , ...   % [m/s2] gravity
    'L_raft'        , L_raft        , ...   % [m] raft length   %'L_domain'      , 5*L_raft      , ...
    'motor_position', 0.00624376948154019, ...*L_raft/2  , ...   % [m] motor x-position (from -L/2 to L/2)
    'd'             , 0.03         , ...   % [m] raft depth (spanwise)
    'EI'            , 0.0422475728604347, ... %3.0e9 * 3e-2 * (9.9e-4)^3 / 12 , ...  % [N m�] bending stiffness
    ...
    'motor_inertia' , 2*0.13e-3*2.5e-3, ...  % [kg m�] motor inertia
